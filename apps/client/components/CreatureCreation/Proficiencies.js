  ...
  return (
    <Dialog sx={{ padding: 4 }} fullWidth open={open} maxWidth={"tablet"} onClose={() => setOpen(false)}>
      ...
      <DialogActions>
        ...
        <Button disabled={!changedSkill} onClick={() => handleSave(option?.key, changedSkill)}>
          Guardar
        </Button>
      </DialogActions>
    </Dialog>
      ...
      <Grid item laptop={12}>
        <Container>
          ...
          <Autocomplete
            ...
            renderInput={(params) => <TextField {...params} variant="outlined" placeholder="Tiradas de salvación" />}
            onChange={(_, newData) => {
      ...
      <Grid item laptop={12}>
        <Container>
          ...
          <Autocomplete
            ...
            filterSelectedOptions
            renderInput={(params) => (
              <TextField {...params} variant="outlined" placeholder="Habilidades con competencia" />
            )}
            onChange={(_, newData) => {
              newData.forEach((element) => {
          ...
          <Autocomplete
            ...
            filterSelectedOptions
            renderInput={(params) => <TextField {...params} variant="outlined" placeholder="Habilidades con pericia" />}
            onChange={(_, newData) => {
              newData.forEach((element) => {
                const key = Object.keys(skills).find((key) => skills[key].name === element);
