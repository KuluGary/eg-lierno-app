...
export default function Details({ creature, setCreature }) {
  ...
  const isNpc = router.pathname.includes("npcs");
  const isMonster = router.pathname.includes("monsters");
  const sizes = [
    ...
    <Grid container spacing={3}>
          ...
          {isCharacter ? (
            ...
            <FormControl fullWidth>
              <Select
                labelId="pronoun-select-label"
                id="pronoun-select"
                value={creature.flavor?.race?.size || sizes[3]}
                onChange={(e) => setCreature("flavor.race.size", e.target.value)}
              >
      ...
      <Grid item laptop={12}>
        <HTMLEditor
          ...
          fullWidth
          label="Descripción"
          value={creature?.flavor?.description ?? ""}
          onChange={(content) => setCreature("flavor.description", content)}
        />
      </Grid>
      {!isMonster && (
        <Grid item laptop={12}>
