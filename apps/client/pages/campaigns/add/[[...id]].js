...
export default function AddCampaign({ campaignData }) {
  ...
  const handleSubmit = () => {
    ...
    if (newCampaign._id) {
      Api.fetchInternal(`/campaigns/${newCampaign._id}`, {
        ...
        .then(() => sendActivationToPlayers(newCampaign._id, newCampaign.players))
        .then(() => router.back());
    } else {
  ...
  const sendActivationToPlayers = (id, players) => {
    const promises = players
      .filter(({ active }) => active === false)
      .map(({ email }) => {
        return Api.fetchInternal(`/campaigns/${id}/send-activation/${email}`, { method: "POST" });
      });
